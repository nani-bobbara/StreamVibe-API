-- =================================================================================
-- MIGRATION: Fix OAuth Blockers (Sprint 3) - REVISED
-- =================================================================================
-- Purpose: Fix 3 critical P0 OAuth blockers preventing platform connections
-- Date: November 11, 2025 (Revised: November 12, 2025)
...
--   #12: Missing schema columns in social_account
-- 
-- Part of:
...
-- 
-- REVISION NOTES:
--   - Fixed total_followers -> total_followers_count (review comment #1)
--   - Added vault_upsert() for reconnection scenarios (review comment #2)
--   - Removed platform TEXT column (conflicts with existing platform_id UUID)
--   - Made platform_user_id nullable (no empty string default)
--   - Removed duplicate columns: followers_count, total_content_count
--   - Reuse existing columns: follower_count, post_count
-- =================================================================================
...
CREATE INDEX IF NOT EXISTS idx_oauth_state_lookup ON public.oauth_state(state, platform) WHERE expires_at > NOW();

-- Index for cleaning up expired states
CREATE INDEX IF NOT EXISTS idx_oauth_state_expired ON public.oauth_state(expires_at) WHERE expires_at <= NOW();
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'oauth_state' AND policyname = 'oauth_state_all_own'
    ) THEN
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE EXCEPTION 'Vault secret with name "%" already exists. Use vault_update or vault_upsert instead.', p_name;
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF FOUND THEN
        v_updated := TRUE;
    ELSE
        RAISE EXCEPTION 'Vault secret "%" not found. Use vault_insert or vault_upsert instead.', p_name;
    END IF;
...
-- Function 5: vault_upsert - Insert or update secret in Vault (for reconnection scenarios)
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM vault.secrets WHERE name = p_name) INTO v_exists;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Failed to upsert secret in Vault: %', SQLERRM;
END;
$$;

COMMENT ON FUNCTION public.vault_upsert IS 'Insert or update encrypted secret in Supabase Vault (handles reconnection scenarios)';
...
GRANT EXECUTE ON FUNCTION public.vault_upsert(TEXT, TEXT) TO service_role;
...
--   - display_name: TEXT (display name from platform)
--   - profile_url: TEXT (public profile URL)
--   - avatar_url: TEXT (avatar/thumbnail URL)
--   - is_active: BOOLEAN (connection status)
--   - last_synced_at: TIMESTAMPTZ (last sync timestamp)
-- 
-- Note: Reuse existing columns where possible:
--   - follower_count (already exists, don't add followers_count)
--   - post_count (already exists, don't add total_content_count)
--   - platform_id (already exists, don't add platform TEXT)
-- =================================================================================

-- Add platform_user_id column (nullable, no empty string default)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.social_account ADD COLUMN platform_user_id TEXT;
...
-- Note: last_synced_at already exists in the table, so we skip adding it

-- Create unique constraint on platform_id + platform_user_id (prevent duplicate connections)
DO $$
BEGIN
    IF NOT EXISTS (
...
ON public.social_account(user_id, platform_id, is_active) 
WHERE deleted_at IS NULL;

-- =================================================================================
-- HELPER FUNCTION: Calculate total follower count across all platforms
-- =================================================================================
-- FIXED: Use correct column name total_followers_count (not total_followers)
-- =================================================================================
...
BEGIN
    ...
    SELECT COALESCE(SUM(follower_count), 0) INTO v_total_followers
    FROM public.social_account
    WHERE user_id = p_user_id
      AND is_active = true
      AND deleted_at IS NULL;
    
    -- Update user profile with total (using correct column name)
    UPDATE public.users
    SET total_followers_count = v_total_followers,
        updated_at = NOW()
    WHERE id = p_user_id;
END;
$$;
...
DECLARE
    ...
    v_vault_upsert_exists BOOLEAN;
    v_platform_user_id_column_exists BOOLEAN;
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM pg_proc 
        WHERE proname = 'vault_upsert' AND pronamespace = 'public'::regnamespace
    ) INTO v_vault_upsert_exists;
    ...
    SELECT EXISTS (
        ...
        AND table_name = 'social_account' 
        AND column_name = 'platform_user_id'
    ) INTO v_platform_user_id_column_exists;
    ...
    RAISE NOTICE '=================================================================================';
    RAISE NOTICE 'MIGRATION VERIFICATION: fix_oauth_blockers (REVISED)';
    RAISE NOTICE '=================================================================================';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '✅ Issue #11: vault_upsert function created (NEW): %', v_vault_upsert_exists;
    RAISE NOTICE '✅ Issue #12: social_account.platform_user_id column added: %', v_platform_user_id_column_exists;
    ...
    IF v_oauth_state_exists AND v_vault_insert_exists AND v_vault_read_exists 
       AND v_vault_update_exists AND v_vault_upsert_exists
       AND v_platform_user_id_column_exists AND v_vault_key_column_exists THEN
        RAISE NOTICE '🎉 ALL OAUTH BLOCKERS FIXED!';
        RAISE NOTICE '';
        RAISE NOTICE '🔧 SCHEMA COMPATIBILITY:';
        RAISE NOTICE '   - Uses existing platform_id (UUID FK) - NO platform TEXT conflict';
        RAISE NOTICE '   - Reuses follower_count - NO followers_count duplication';
        RAISE NOTICE '   - Reuses post_count - NO total_content_count duplication';
        RAISE NOTICE '   - Fixed total_followers_count column name';
        RAISE NOTICE '   - Added vault_upsert() for reconnection scenarios';
        RAISE NOTICE '';
        RAISE NOTICE '🧪 Test OAuth Flow:';
        RAISE NOTICE '   1. Deploy Edge Functions: supabase functions deploy';
        RAISE NOTICE '   2. Update Edge Functions to use vault_upsert() instead of vault_insert()';
        RAISE NOTICE '   3. Test YouTube OAuth: GET /functions/v1/oauth-youtube-init';
        RAISE NOTICE '   4. Complete OAuth in browser (redirects to Google)';
        RAISE NOTICE '   5. Verify oauth_state record created and deleted';
        RAISE NOTICE '   6. Verify tokens stored in Vault (use vault_read)';
        RAISE NOTICE '   7. Verify social_account created with channel info';
        RAISE NOTICE '   8. Test reconnection (should use vault_upsert)';
        RAISE NOTICE '';
